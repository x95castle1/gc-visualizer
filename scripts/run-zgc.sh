#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
# run-zgc.sh  —  Launch GC Visualizer with ZGC (Generational, Java 21)
#
...
echo " ╚══════╝ ╚═════╝  ╚═════╝"
echo ""
echo " GC: ZGC (Generational)  |  Profile: $PROFILE  |  Heap: $XMX"
echo " Dashboard: http://localhost:8080"
echo " GC Log:    logs/zgc-$PROFILE.log"
echo ""
...
java \
  ...
  -XX:+ZGenerational \
  $XMX \
  -Xlog:gc*:file=logs/zgc-$PROFILE.log:time,uptime,level,tags \
  -jar "$JAR"
